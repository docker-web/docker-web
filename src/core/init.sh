INIT() {
  ...
  sed -i "s|__SERVICE_NAME__|$NAME|g" $FOLDER/docker-compose.yml
  sed -i "s|__SERVICE_NAME__|$NAME|g" $FOLDER/README.md
  sed -i "s|__SERVICE_NAME__|$NAME|g" $FOLDER/config.sh
