...
sed -i "s:LANGUAGE_LANG:$LANGUAGE_LANG:g" "$PATH_DOCKERWEB_APPS/$1/librarie.json"
sed -i "s:LANGUAGE_LOCAL:$LANGUAGE_LOCAL:g" "$PATH_DOCKERWEB_APPS/$1/librarie.json"
...
# set Librarie
curl -H "Content-Type: application/json" -d "$PATH_DOCKERWEB_APPS/$1/librarie.json" "${DOMAIN}/Library/VirtualFolders?collectionType=movies&refreshLibrary=false&name=Movies"
